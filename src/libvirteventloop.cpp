...
int LibvirtEventLoop::addHandle(int fd, int events, virEventHandleCallback cb, void *opaque, virFreeCallback ff)
{
    ...
    if (events & VIR_EVENT_HANDLE_READABLE) {
        ...
        QObject::connect(readNotifier, &QSocketNotifier::activated, [=](int socket) {
            cb(watch, socket, events, opaque);
        });
    ...
    if (events & VIR_EVENT_HANDLE_WRITABLE) {
        ...
        QObject::connect(writeNotifier, &QSocketNotifier::activated, [=](int socket) {
            cb(watch, socket, events, opaque);
        });

...
DomainViewer::DomainViewer(QQuickItem *parent) : QQuickItem(parent),
                                                 m_domain(nullptr),
                                                 m_host(QStringLiteral("localhost")),  // Default host
                                                 m_port(5900),
                                                 m_connected(false),
...
QSGNode *DomainViewer::updatePaintNode(QSGNode *oldNode, UpdatePaintNodeData *) {
    ...
    QSGTexture *texture = window()->createTextureFromImage(m_frameBuffer, QQuickWindow::TextureHasAlphaChannel);
    if (texture) {
...
void DomainViewer::channel_new_cb(SpiceSession *session, SpiceChannel *channel, gpointer user_data) {
    ...
    if (SPICE_IS_DISPLAY_CHANNEL(channel)) {
        ...
        g_signal_connect(channel, "display-primary-create",
                         G_CALLBACK(display_primary_create_callback), item);
        g_signal_connect(channel, "display-invalidate",
                         G_CALLBACK(display_invalidate_callback), item);
    }
}
void DomainViewer::display_primary_create_callback(SpiceChannel *channel,
    ...
    item->m_frameBuffer = QImage((uchar *)imgdata, width, height, stride, QImage::Format_RGB32);
    item->m_frameUpdated = true;

    QMetaObject::invokeMethod(item, "frameUpdated", Qt::QueuedConnection);
    QMetaObject::invokeMethod(item, "update", Qt::QueuedConnection);
}

void DomainViewer::display_invalidate_callback(SpiceDisplayChannel *channel,
                                               gint x, gint y,
                                               gint width, gint height,
                                               gpointer user_data) {
    DomainViewer *item = static_cast<DomainViewer *>(user_data);
...
void DomainViewer::checkChannelStatus() {
    ...
    if (!hasDisplayChannel) {
        qCWarning(KARTON_DEBUG) << "No display channel found after"
                                << (m_connected ? "successful connection!" : "failed connection");
    }
}
